    ...
    if pressed then
        ...
        if button['_held'] ~= nil then
            ...
            if button['held'] then
                ...
                local disabled_image = button_image_from_file("disabled.png")
...
function button_image_from_file(filename, frame)
    local composite = composite_image_from_file(filename)
    local target_frame = frame or { x = 0, y = 0, w = stream_deck_button_pixels, h = stream_deck_button_pixels }
    local elements = {{
        type = "image",
        image = composite,
        frame = target_frame,
...
function solid_colour_fill(colour)
    if type(colour) == "string" then
        colour = hammerspoon_colour(colour)
    end
